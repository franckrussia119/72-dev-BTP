    ...
    <>
      ...
      <div className="page-header">
        ...
        <div className="page-header-content">
          ...
          <div>
            <h1 style={{ fontFamily: 'Nunito Sans', fontSize: 'clamp(1.8rem, 3vw, 2.5rem)', fontWeight: 800, color: 'white', marginBottom: '0.5rem' }}>Portfolio</h1>
      ...
      <section style={{ padding: '5rem 5%', background: 'white' }}>
        <div style={{ maxWidth: '1280px', margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '4rem', alignItems: 'start' }}>
          <div>
            <h2 style={{ fontFamily: 'Nunito Sans', fontWeight: 800, fontSize: '1.5rem', color: '#1a2b4a', marginBottom: '1rem', lineHeight: 1.3 }}>Exterior design projects</h2>
          ...
          <div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px', marginBottom: '8px' }}>
              {projects.slice(0, 3).map((src, i) => (
                <img key={i} src={src} alt={`Project ${i + 1}`} style={{ width: '100%', height: '150px', objectFit: 'cover' }} />
              ))}
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '8px' }}>
              {projects.slice(3).map((src, i) => (
                <img key={i} src={src} alt={`Project ${i + 4}`} style={{ width: '100%', height: '150px', objectFit: 'cover' }} />
              ))}
            </div>
          </div>
        </div>
      ...
      <section style={{ padding: '5rem 5%', background: '#f4f6f8' }}>
        <div style={{ maxWidth: '1280px', margin: '0 auto', display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '4rem', alignItems: 'start' }}>
          <div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px', marginBottom: '8px' }}>
              {[
                'https://72dev-btp.com/wp-content/uploads/2025/02/Highway_Construction.avif',
                ...
                <img key={i} src={src} alt={`Road ${i + 1}`} style={{ width: '100%', height: '150px', objectFit: 'cover' }} />
              ))}
            </div>
          </div>
          <div>
            <h2 style={{ fontFamily: 'Nunito Sans', fontWeight: 800, fontSize: '1.5rem', color: '#1a2b4a', marginBottom: '1rem', lineHeight: 1.3 }}>Road & Infrastructure projects</h2>
            <p style={{ color: '#7a8a9a', lineHeight: 1.8, fontSize: '0.9rem' }}>
              Building durable connections across African nations. Our road and infrastructure projects span highways, rural paths, and major bridges — all engineered for Africa's unique terrain and climate.
            </p>
          </div>
        </div>
      </section>
      ...
      <section style={{ position: 'relative' }}>
        ...
        <div style={{ background: '#1e3a4a', padding: '5rem 5%', textAlign: 'center' }}>
          <h2 style={{ fontFamily: 'Nunito Sans', color: 'white', fontSize: '1.8rem', fontWeight: 800, marginBottom: '1rem' }}>Have a Project in Mind?</h2>
